...
import mockActivitiesPercentageData from 'src/components/widgets/ActivitiesPercentage/mockActivitiesPercentageData.json';
...
describe('Pie React-Redux Component', () => {
  it('Matches canvas snapshot', async () => {
    ...
    page.on('request', (request) => {
      ...
      } else if (request.url().includes('widget/ActivitiesPercentage')) {
        void request.respond({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify(mockActivitiesPercentageData)
        });
      } else {
